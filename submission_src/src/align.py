...
def normalize_volume(vol):
    """Match train_vol3d.normalize: percentile clip + min-max to [0,1]."""
    lo, hi = np.percentile(vol, 1.0), np.percentile(vol, 99.5)
    vol = np.clip(vol, lo, hi)
    vol = (vol - lo) / max(hi - lo, 1e-8)
...
def load_aligned_mips(nifti_path, use_cache=True):
    if not use_cache:
        v, s = read_nifti(nifti_path)
        return aligned_mips(v, s)
    return _cache_load(nifti_path, CACHE_MIPS,
                       lambda p: aligned_mips(*read_nifti(p)), suffix=f"t{MIP_TARGET}")